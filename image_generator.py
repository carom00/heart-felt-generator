...
def execute(text, fileName): 
    ...
    input_images = ""
    #result = summ_model(text, num_sentences = 3)

    summ_model = Summarizer()
    result = summ_model(text, ratio = 0.3)
    ...
    def checkin(i, losses):
        losses_str = ', '.join(f'{loss.item():g}' for loss in losses)
        out = synth(z)
        TF.to_pil_image(out[0].cpu()).save(fileName + '.png')
        add_stegano_data(fileName + '.png')
        add_xmp_data(fileName + '.png')
    ...
    def train(i):
        ...
        lossSum = sum(lossAll)
        loss = lossSum
        torch.autograd.backward(lossAll)
        opt.step()
        with torch.no_grad():
